...
class UserInfoApi {
  async getUserList(pageNum) {
    ...
    const response = await axios.get(
      `${SERVER}/admin/user/general?page=${pageNum}&size=${size}&sort=id,desc`
    );
  ...
  async getAdminList(pageNum) {
    ...
    const response = await axios.get(
      `${SERVER}/admin/user/adminUser?page=${pageNum}&size=${size}&sort=id,desc`
    );
  ...
  async changeRoleUser(id) {
    const response = await axios.delete(
      `${SERVER}/admin/user/permission/${id}`
    );
  }

  async searchForName(pageNum, name) {
    const size = 5;
    const response = await axios.get(`${SERVER}/admin/user/search`, {
      params: {
        page: pageNum,
        size: size,
        sort: "id,desc",
